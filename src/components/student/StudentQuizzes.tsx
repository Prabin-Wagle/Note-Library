...
const StudentQuizzes: React.FC = () => {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      ...
      try {
        ...
        // Fetch quizzes matching the specified grades, ordered by creation date
        const q = query(
  ...
  if (currentView === 'series' && selectedSeries) {
    const seriesQuizzes = quizzes.filter(quiz => quiz.grade === selectedSeries);
    ...
    return (
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        {seriesQuizzes.length === 0 ? (
          <p className="text-center text-gray-600">No quizzes available for {selectedSeries} at the moment.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {seriesQuizzes.map((quiz) => (
              <div key={quiz.id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between">
                <div>
                  <h2 className="text-xl font-semibold text-sky-600 mb-2">{quiz.title}</h2>
